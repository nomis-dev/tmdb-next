  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center h-[50vh] text-center px-4">
        <h1 className="text-2xl font-bold mb-4 text-white">{t('pleaseSignIn')}</h1>
        <p className="text-slate-400 mb-8">{t('accessList')}</p>
      </div>
    );
  }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 mt-20 md:mt-16">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">
        ...
        <div className="flex items-center gap-3">
          <span className="text-sm text-slate-400 mr-1">{t('sortBy')}</span>
      ...
      {favorites.length === 0 ? (
        <div className="text-center py-20 bg-slate-900/50 rounded-2xl border border-white/5">
          ...
          <p className="text-slate-400 mb-6">{t('startExploring')}</p>
          <Link
            href="/"
